...
module.exports = (agendash, options) => {
  const { api, requeueJobs, deleteJobs, createJob, stopJobs } = agendash;
  const app = express();
  ...
  if(options.apiMiddleware.length){
    app.use(options.apiMiddleware)
  }
  ...
  app.post("/api/jobs/stop", async (request, response) => {
    try {
      const stopped = await stopJobs(request.body.jobIds);
      if (stopped) {
        response.json({ stopped: true });
      } else {
        response.json({ message: "Jobs not stopped" });
      }
    } catch (error) {
      console.log(error)
      response.status(404).json(error);
